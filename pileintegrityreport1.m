function [  ] = pileintegrityreport1( varargin )
%   PileIntegrityReport  创建桩身完整性报告,v20200817
%   按报告编号，以每份输出
%   目录下必须有“检测报告”文件夹，“报告中引用文件”中须有相关引用文件存在
%   pileintegrityreport1( 2 )   可输入1个正整数，表示输出‘序号’2的检测报告
%   pileintegrityreport1( 2, 8 )   可输入2个正整数，表示输出‘序号’2~8的检测报告
...
disp('输出报告封面扉页')
cover1( beginNo , endNo ,  logospc , logosize , exportAsPDF , exportFlyPage , ...
...
disp('输出报告页')
reportPage1( beginNo , endNo , scgsytspec_3 , scgsytspec_4 , exportAsPDF , supervisionPattern , ...                      %   报告页输出函数
...
function [] = cover1( beginNo , endNo ,  logospc , logosize , exportAsPDF , exportFlyPage , ...
...
for ii = beginNo : 1 : endNo
    ...
    disp(['    输出序号 ', bgxh{ii} ,' 报告编号( ' , bgbh{ii} ,' )封面扉页']);
    ...
    selection.ParagraphFormat.SpaceBefore = 5;                               %  段前间距(磅)
    selection.ParagraphFormat.SpaceAfter = 10;                               %  段后间距(磅)
    selection.MoveDown;
...
%     dd = num2str( day( bgrq{ii} ) );
    ...
    elseif exportFlyPage == 0
        ...
        error('输出封面扉页“注意事项” 选择错误');
    end
    
    
    %%%%%%  保存文件  %%%%%%
    disp('        保存封面扉页文档');
    bgmc = strcat('封面扉页' , bgbh{ii});                                         %  输出检测报告名称
    filespec= fullfile(pwd ,'\检测报告\' , bgmc);
    try
...
function [] = reportPage1( beginNo , endNo , scgsytspec_3 , scgsytspec_4 , exportAsPDF , supervisionPattern , ...                      %   报告页输出函数
    bgxh , bgbh , jgwmc , dtbh , jzbh , concreteStrength , sjzc , sczc , sjzj , gzfl , gzrq , jcrq , jcjg , ...
    qxfx , cggs , cgjj1_2 , cgjj1_3 , cgjj2_3 , cgjj1_4 , cgjj2_4 , cgjj3_4 , fjys , ...
    cdjj , yqsb , jcbz , jzbg , gcmc , wtdw , jcnr , jclb , bxry , jcry , fhry , ...
    shry , pzry , pzjb , jcdw , dwdz , dwdh , yzbm , dwcz , bgrq , bgys , ...
...
set(word,'Visible',0);                                                      %  设置服务器界面变为可见状态
...
for ii = beginNo : 1 : endNo
    ...
    DTI.Borders.InsideLineStyle = 'wdLineStyleSingle';                      %  设置内边框线型
    DTI.Borders.InsideLineWidth = 'wdLineWidth050pt';                       %  设置内边框线宽
    ...
    DTI.Cell(6,1).Range.Text = '报 告 编 写';
    ...
    selection.TypeParagraph;                                                %  回车，插入一行
    pilepositionfilespec = fullfile(pwd,'报告中引用文件\',zwpmbzt{ii});      %  图1 桩位平面布置图的文件位置
    ...
    for mm = 1 : 2 : document.Sections.Count                                    %  中间数2为每2段执行一次循环
        ...
        document.Sections.Item(mm+1).Headers.Item('wdHeaderFooterPrimary').Range.Text = strcat(jcnr{ii},'报告',09,bgbh{ii});                         %   strcat( '混凝土基桩完整性超声波检测',09,bgbh{ii});         %   设置第二节页眉内容
    ...
    for nn = 1 : 2 : document.Sections.Count
        ...
        selection.Range.InsertAfter(strcat( '页 共',bgys{ii}, '页'));
    ...
    %%%%%%  保存文件  %%%%%%
    disp('        保存文档');
    bgmc = strcat('报告页' , bgbh{ii});                                         %  输出检测报告名称
    filespec= fullfile(pwd,'\检测报告\',bgmc);
    try
        document.SaveAs(filespec);
    catch
...
fhry = cell(raw2_m - 1, 1);      %  复核
shry = cell(raw2_m - 1, 1);      %  审核
